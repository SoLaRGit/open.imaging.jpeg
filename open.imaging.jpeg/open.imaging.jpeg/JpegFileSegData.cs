...
namespace open.imaging.jpeg
{
  ...
  public class JpegFileSegData : JpegFileSeg
  {
    internal JpegFileSegData()
    { }

    public JpegFileSegData(JpegFile jpegFile, JpegFileSegMarker id, JpegBinaryReader reader)
    ...
    public override bool Read(JpegBinaryReader reader)
    {
      ...
      if (size > 0) data = reader.read_u1(length);        // read data (size-2) bytes
      return true;
    }
    ...
    public override int Compare(JpegFileSeg other)
    {
      ...
      if (null == other.AsDATA)
      {
        throw (new ArgumentNullException("JpegFileSeg other argument must be of type JpegFileSegData."));
      }
    ...
    {
      return string.Format("JpegFileSegData\t:\t{{ a: 0x{0,8:x8}, id: {1,-5} ({2,4:X4}.h), size: {3,5} (0x{4,4:x4}) }}",
                            _address,
                            MarkerId,
                            ((ushort)MarkerId),
                            size,
